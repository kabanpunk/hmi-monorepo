<template>
  <div v-bind="$attrs">
    ...
    <n-button block tertiary size="small" @click="openFile">
      Загрузить...
    </n-button>
    <input
        type="file"
...
function doSave() {
  ...
  const hmi = {
    ...
    bindings: canvas.value.getObjects().map((o: fabric.Object & { bindings?: any; id?: string }) => ({
      elementId: o.id,
...
function onFile(e: Event) {
  ...
  f.text().then(text => {
    ...
    fabric.util.enlivenObjects(json.canvas.objects, (objs: fabric.Object[]) => {
      objs.forEach((o: fabric.Object) => canvas.value!.add(o))
